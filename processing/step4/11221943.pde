...
class Particle {
  ...
  Particle(PVector l, float _r, float vx, float vy, float ax, float ay, boolean isFRun) {
...
void addParticle() {
    if ( mousecompare()) {
      ...
      particles.add(new Particle (origin, r, vx, vy, ax, ay, true));
    }
    if (!(mousecompare())) {
      vx= 0; 
      vy= random(0, 1);
      ax = random(-0.02, 0.04); 
      ay = random(0.1);
      particles.add(new Particle (origin, r, vx, vy, ax, ay, false));
